...
# Two T4 sessions split ALL generators by seed parity and together cover all
# 180 configurations with no overlap. The whole experiment is trained on T4:
# REQUIRE_TRAINED_ON_GPU makes a session retrain from scratch any configuration
# of its shard whose checkpoint came from another GPU (the earlier A100 session).
_T4 = {"REQUIRE_TRAINED_ON_GPU": '"T4"'}
VARIANTS = {
    "run_T4_1": ({"SHARD_SEEDS": "(0, 2, 4, 6, 8)", **_T4},
                 "seeds 0, 2, 4, 6, 8 of GBM, Heston and SBTS", "T4"),
    "run_T4_2": ({"SHARD_SEEDS": "(1, 3, 5, 7, 9)", **_T4},
...
> that are already complete are skipped — unless their checkpoint was trained
> on a GPU other than T4, in which case they are trained again from scratch
> and the notebook lists them before the queue starts.
>
> **Whichever of the two finishes LAST carries straight on into the audit,
> evaluation, statistics (MCS), diagnostics and tables. There is no third
> notebook.** The one that finishes first stops after Cell 19 with
> `ShardTrainingComplete`; that is the expected end, not an error. If both stop
> without the analysis — possible because Google Drive syncs between machines
> with a delay — run either one again: it finds all 180 complete and goes
> straight to the analysis.
>
> Both join the existing run automatically (same `config_hash`, most completed
> configurations) and must print the same `run_id` in Cell 3. A finished
